...
if ($action == 'add') {
    ...
    $sql = 'INSERT INTO tb_user (user_username, user_password, user_title, user_fname, user_lname, user_position, user_address, user_tel, user_email, role_id) VALUES ("' . $user_username . '", "' . $user_password . '", "' . $user_title . '", "' . $user_fname . '", "' . $user_lname . '", "' . $user_position . '", "' . $user_address . '", "' . $user_tel . '", "' . $user_email . '", "' . $role_id . '")';
...
if ($action == 'listadvisor') {
    $field_id = $_POST['field_id'];

    $sql = 'SELECT * FROM tb_user WHERE field_id = "' . $field_id . '"';
    $result = mysqli_query($con, $sql);
    while ($r = mysqli_fetch_array($result)) {
        $arr[] = array("user_id" => $r['user_id'], "user_fullname" => $r['user_title'] . $r['user_fname'] . ' ' . $r['user_lname']);
    }
