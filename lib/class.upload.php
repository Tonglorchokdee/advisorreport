...
class upload {
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        $this->translation['tuser_file_missing']           = 'No correct temp source file. Can\'t carry on a process.';
        ...
        $this->translation['tuser_file']                   = 'Can\'t create the temporary file. Can\'t carry on a process.';
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if (substr($file, 0, 4) == 'php:') {
                    ...
                    $hash = $this->tuser_dir() . md5($file . rand(1, 1000));
                    if (file_put_contents($hash, file_get_contents('php://input'))) {
                        ...
                        $this->log .= ' failed<br />';
                        $this->uploaded = false;
                        $this->error = $this->translate('tuser_file');
                    }
    ...
    function tuser_dir() {
        $dir = '';
        if (function_exists('sys_get_tuser_dir')) $dir = sys_get_tuser_dir();
        ...
        if ($this->processed) {
            ...
            if (!empty($this->file_src_temp)) {
                ...
                if (!file_exists($this->file_src_pathname)) {
                    $this->processed = false;
                    $this->error = $this->translate('tuser_file_missing');
                }
